...
const futureSkuMap = new Map<string, string>();

for (const row of rows) {
  const sku = String(row["SKU"] ?? "").trim().toLowerCase();
  const futureSku = String(row["Future SKU"] ?? "").trim().toLowerCase();

  if (futureSku) {
    futureSkuMap.set(futureSku, sku);
  }
}
...
const matchedSlugProduct =
  !matchedSkuProduct ? slugMap.get(slug) : undefined;
...
const matchedBySku = !!matchedSkuProduct;
const matchedBySlug = !!matchedSlugProduct;
const matchedByName = !!matchedNameProduct;

const matchType = matchedBySku
  ? "SKU"
  : matchedBySlug
  ? "SLUG"
  : matchedByName
...
console.log({
  ...
  updates: syncItems.filter((i) => i.action === "UPDATE").length,
  inserts: syncItems.filter((i) => i.action === "INSERT").length,
  reviews: syncItems.filter((i) => i.action === "REVIEW").length,
});
